...
app.get("/verifyEmail/:id", (req, res) => {
  ...
  jwt.verify(token, "GYANPRAKASHGPS", async (err, verifiedJwt) => {
    if (err) res.send(err);
...
app.post("/forget",async(req,res)=>{
  ...
  if(password.length<5 || password!==confirmPassword || !validator.isEmail(email)){
    return res.send({
      status:400,
      message:"Something id wrong "
    })
  }
  ...
  let userExists;
  try {
    userExists = await UserSchema.findOneAndUpdate({ email });
  } catch (err) {
    return res.send({
      status: 400,
      message: "Internal server error, Please try again",
  ...
  console.log(verificationToken);
  try {
    let userDB=await UserSchema.findOneAndUpdate({ email },{"$set":{emailAuthenticated:false,password:hashedPassword}}); // create opt in database
